...
public:
	...
	static string GetMonthCalendar(short Year, short Month, int Line) {
		...
		short FirstDay = DayOfWeekOrder(Year, Month, 1) + 1;
	...
	static void PrintMonthCalender(clsDate Date)
	{
		...
		else
		{
			...
			short DayOre = DayOfWeekOrder(Date._Year, Date._Month, 1);
	...
	static string GetTimeOfThisMomentString()
	{
	...
	static string GetDateAndTimeOfThisMomentSrting()
	{
		short Hour = 0, Min = 0, Sec = 0;
		string DateTime = "";
		time_t t = time(0);
		...
		DateTime = DateToString(clsDate());

		DateTime += " - ";
		if (Hour < 10)
		{
			DateTime += "0" + to_string(Hour);
		}
		else
		...
		if (Min < 10)
		{
			DateTime += ":0" + to_string(Min);
		}
		else
			DateTime += ":" + to_string(Min);

		if (Sec < 10)
		{
			DateTime += ":0" + to_string(Sec);
		}
		else
			DateTime += ":" + to_string(Sec);


		return DateTime;
	}

	static bool IsDateAndTimeBeforeTheMomentDateAndTime(clsDate Date, vector<short> Time)
	{
		if (IsDate1BeforeDate2( clsDate(),Date))
		{
			return true;
		}
		else if (IsDate1AfterDate2(clsDate(), Date))
		{
			return false;
		}
		else
		{
			short AtHour = 0, AtMin = 0, AtSec = 0;
			string Time = "";
			time_t t = time(0);
			tm* now = localtime(&t);
			AtHour = now->tm_hour;
	...
	static int NumberOfSecondsToDateAndTimeEqualTheMomentDateAndTime(clsDate Date,  vector<short> Time)
	{
		
		if (!IsDateAndTimeBeforeTheMomentDateAndTime(Date, Time))
		{
			return 0;
		}
...
//		string Time = "";
		...
		int TotalSeconds = 0;
		int Days = GetDifferenceInDays( clsDate(), Date)  ;
		...
		TotalSeconds = (Days * 86400) - ((AtHour * 3600) + (AtMin * 60) + (AtSec)) + ((Time[0] * 3600) + (Time[1] * 60) + (Time[2]));
